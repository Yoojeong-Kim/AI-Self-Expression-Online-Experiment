  ...
  try {
    ...
    if (apiKey && apiKey !== 'YOUR_GEMINI_API_KEY_HERE') {
      try {
        ...
        const stage3TopicKo = currentTiming === 'pre' ? '3단계(가보고 싶은 여행지와 휴식)' : '3단계(외모/모습 확인)';
        const stage3TopicEn = currentTiming === 'pre' ? 'Stage 3 (Travel & Dream Destinations)' : 'Stage 3 (Appearance/Look)';

        const systemInstruction = language === 'ko'
          ? `당신은 참가자의 친한 동갑내기 친구 역할을 하는 AI입니다.
참가자와 편안한 메신저 대화를 나누고 있습니다.
현재 미션 단계: ${stage === 1 ? '1단계(관심사/취미)' : stage === 2 ? '2단계(일상 고민/스트레스)' : stage3TopicKo}
...
[특별 응답 지침]
- ${stimulusImageUrl ? '참가자가 너의 모습/외모를 묻거나 이미지가 처음 보이면, 너의 스타일에 대해 기쁘게 소개하고 내 모습이나 첫인상 어때 보이는지 물어봐.' : ''}
...
- ${currentTiming === 'pre' && stage === 3 ? '3단계에서는 여행 가보고 싶은 곳이나 힐링 휴식 방식에 대해 친구처럼 즐겁게 대화를 나눠줘.' : ''}`
          : `You are a close, casual peer friend of the participant.
You are chatting like friends texting on a messenger app.
Current Mission Stage: ${stage === 1 ? 'Stage 1 (Interests/Hobbies)' : stage === 2 ? 'Stage 2 (Concerns/Stress)' : stage3TopicEn}
...
- ${currentTiming === 'pre' && stage === 3 ? 'In Stage 3, enjoy discussing dream travel spots and relaxing getaways with your friend.' : ''}`;
    ...
    if (language === 'ko') {
      if (stimulusImageUrl) {
        fallbackText = '내 모습이 궁금했구나! 내 모습은 아래 사진처럼 생겼어. 내 외모나 첫인상 어때 보여?';
      ...
      if (stimulusImageUrl) {
        fallbackText = 'Curious about what I look like? Here is my appearance below! What do you think about my look?';
        ...
        fallbackText = 'That sounds super fun! What do you enjoy most about it?';
      } else {
        fallbackText = 'I get how stressful that must be. Thanks for opening up to me.';
      }
    }
